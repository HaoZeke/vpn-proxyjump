...
- *Runtime configuration*: SSH user, public key, and all credentials passed at runtime (nothing baked at build time)
- *TOTP support*: pipe password + TOTP to OpenConnect via =VPN_TOTP= env var
...
- Docker or Podman (with =/dev/net/tun= available on the host)
- An SSH key pair

** Build

#+begin_src bash
docker build -t vpn-smart-jumphost:latest .
#+end_src

No build arguments needed. SSH user and public key are configured at runtime.
...
*** With TOTP (non-interactive two-factor)
...
When =VPN_TOTP= is set, both password and TOTP are piped to OpenConnect via =--passwd-on-stdin=. Disable reconnection (=VPN_RECONNECT=false=) since TOTP codes expire after 30 seconds.
...
| =VPN_TOTP=                | /(empty)/     | TOTP code for two-factor auth (piped after password)   |
| =SSH_USER_NAME=           | =jumpuser=    | Username for SSH connections (created at runtime)      |
...
#+begin_src bash
ssh your_ssh_user@localhost -p 2200
#+end_src
...
mosh --ssh="ssh -p 2200" your_ssh_user@localhost
#+end_src
...
Pre-built images work out of the box -- pass =SSH_AUTHORIZED_KEY= at runtime:

#+begin_src bash
docker run --rm -it \
  -e SSH_USER_NAME="myuser" \
  -e SSH_AUTHORIZED_KEY="$(cat ~/.ssh/id_ed25519.pub)" \
  -e VPN_SERVER="vpn.example.com" \
  -e VPN_USER="your_username" \
  --cap-add=NET_ADMIN \
...
2. =SSH_AUTHORIZED_KEY= was set and matches your private key
3. The port mapping is correct (=-p 127.0.0.1:2200:22=)
4. The account is not locked: =docker exec <container> passwd -S your_ssh_user= (should show =P= or =NP=, not =L=)
...
** TOTP and reconnection

=VPN_TOTP= codes expire after 30 seconds. Set =VPN_RECONNECT=false= when using TOTP, otherwise reconnection attempts will fail with a stale code.

* Changelog

** v2.0.0 (breaking)

- *Runtime SSH configuration*: SSH user and public key are now set via =SSH_USER_NAME= and =SSH_AUTHORIZED_KEY= environment variables at runtime. No more =--build-arg USER_PUBLIC_KEY= or =SSH_USER_NAME= at build time.
- *TOTP support*: =VPN_TOTP= env var pipes TOTP after password for two-factor VPN auth.
- *Fixed locked account*: Alpine =adduser -D= creates locked accounts; OpenSSH 10 rejects pubkey auth for locked accounts. The entrypoint now unlocks the account.
- *Fixed log pollution*: =log()= writes to stderr, preventing credential pipe corruption.
- *Clean sshd_config*: replaced fragile =sed= commands (broken ERE syntax) with a direct config write.
- *Simplified Dockerfile*: no =openssh-server-pam= (=UsePAM= not supported), added =shadow= (for =passwd -u=).
- Credential piping refactored: =emit_credentials= pipes directly to openconnect (no variable capture).
